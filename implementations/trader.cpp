...
size_t Trader::get_id() const { return trader_id; }
double Trader::get_fund() const { return fund; }

void Trader::place_sell_order(Stock& stock, unsigned int quantity, double min_price) {
    ...
    if (trader_strategy == Strategy::LIMIT_ORDER)
        order = std::make_unique<LimitSellOrder>(*this, stock, quantity,min_price);
    else if (trader_strategy == Strategy::MARKET_ORDER)
        order = std::make_unique<MarketSellOrder>(*this, stock,quantity,this->get_fund());
    else
...
void Trader::place_buy_order(Stock& stock, unsigned int quantity, double max_price) {
    std::unique_ptr<Order> order;
    if (trader_strategy == Strategy::LIMIT_ORDER)
        order = std::make_unique<LimitBuyOrder>(*this,stock,quantity,max_price);
    else if (trader_strategy == Strategy::MARKET_ORDER)
        order = std::make_unique<MarketBuyOrder>(*this,stock,quantity,this->get_fund());
    else
        throw std::runtime_error("no strategy chosen");
